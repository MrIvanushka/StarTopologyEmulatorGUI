#include "EnergyAwareIncomeLoadConfigWidget.h"
...
EnergyAwareIncomeLoadConfigWidget::EnergyAwareIncomeLoadConfigWidget(QWidget* parent)
	: QWidget(parent)
{
	setupUi();
}

EnergyAwareLoadControllerConfig EnergyAwareIncomeLoadConfigWidget::getConfig() const
{
	EnergyAwareLoadControllerConfig cfg;
	cfg.weightThroughput = _weightThroughputSpin->value();
	cfg.weightEnergy = _weightEnergySpin->value();
...
void EnergyAwareIncomeLoadConfigWidget::setConfig(const EnergyAwareLoadControllerConfig& cfg)
{
	_weightThroughputSpin->setValue(cfg.weightThroughput);
	_weightEnergySpin->setValue(cfg.weightEnergy);
...
void EnergyAwareIncomeLoadConfigWidget::setupUi()
{
	...
	auto* utilGroup = new QGroupBox(QString::fromLocal8Bit("Параметры энергоэффективной полезности"), this);
	auto* utilLayout = new QFormLayout(utilGroup);
	_weightThroughputSpin = new QDoubleSpinBox();
	_weightEnergySpin = new QDoubleSpinBox();
	_gradientStepSpin = new QDoubleSpinBox();
	setupDouble(_weightThroughputSpin, 0.0, 1000.0, 0.1, 3);
	setupDouble(_weightEnergySpin, 0.0, 1000.0, 0.1, 3);
	setupDouble(_gradientStepSpin, 0.0, 10.0, 0.01, 4);
	utilLayout->addRow(QString::fromLocal8Bit("Вес пропускной способности (wThroughput): "), _weightThroughputSpin);
	utilLayout->addRow(QString::fromLocal8Bit("Вес энергопотребления (wEnergy): "), _weightEnergySpin);
	...
	setConfig(EnergyAwareLoadControllerConfig());
}
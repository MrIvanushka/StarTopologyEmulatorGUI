#include "SimpleUtilityBasedIncomeLoadConfigWidget.h"
...
SimpleUtilityBasedIncomeLoadConfigWidget::SimpleUtilityBasedIncomeLoadConfigWidget(QWidget* parent)
	: QWidget(parent)
{
	setupUi();
}

SimpleMarginalUtilityBasedLoadControllerConfig SimpleUtilityBasedIncomeLoadConfigWidget::getConfig() const
{
	SimpleMarginalUtilityBasedLoadControllerConfig cfg;
	cfg.weightThroughput = _weightThroughputSpin->value();
	cfg.weightCollision = _weightCollisionSpin->value();
...
void SimpleUtilityBasedIncomeLoadConfigWidget::setConfig(const SimpleMarginalUtilityBasedLoadControllerConfig& cfg)
{
	_weightThroughputSpin->setValue(cfg.weightThroughput);
	_weightCollisionSpin->setValue(cfg.weightCollision);
...
void SimpleUtilityBasedIncomeLoadConfigWidget::setupUi()
{
	auto setupDouble = [](QDoubleSpinBox* sb, double min, double max, double step, int decimals = 4) {
		sb->setRange(min, max);
	...
	auto* utilityGroup = new QGroupBox(QString::fromLocal8Bit("Функция полезности (NUM)"), this);
	auto* utilityLayout = new QFormLayout(utilityGroup);

	_weightThroughputSpin = new QDoubleSpinBox();
	_weightCollisionSpin = new QDoubleSpinBox();
	_gradientStepSpin = new QDoubleSpinBox();
	setupDouble(_weightThroughputSpin, 0.0, 1000.0, 0.1, 3);
	setupDouble(_weightCollisionSpin, 0.0, 1000.0, 0.1, 3);
	setupDouble(_gradientStepSpin, 0.0, 10.0, 0.01, 4);
	utilityLayout->addRow(QString::fromLocal8Bit("Вес полезной пропускной способности (wThroughput): "), _weightThroughputSpin);
	utilityLayout->addRow(QString::fromLocal8Bit("Вес коллизий (wCollision): "), _weightCollisionSpin);
	...
	setConfig(SimpleMarginalUtilityBasedLoadControllerConfig());
}
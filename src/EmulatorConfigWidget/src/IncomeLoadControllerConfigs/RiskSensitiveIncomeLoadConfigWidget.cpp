#include "RiskSensitiveIncomeLoadConfigWidget.h"
...
RiskSensitiveIncomeLoadConfigWidget::RiskSensitiveIncomeLoadConfigWidget(QWidget* parent)
	: QWidget(parent)
{
	setupUi();
}

RiskSensitiveLoadControllerConfig RiskSensitiveIncomeLoadConfigWidget::getConfig() const
{
	RiskSensitiveLoadControllerConfig cfg;
	cfg.beta = _betaSpin->value();
	cfg.collisionPenalty = _collisionPenaltySpin->value();
...
void RiskSensitiveIncomeLoadConfigWidget::setConfig(const RiskSensitiveLoadControllerConfig& cfg)
{
	_betaSpin->setValue(cfg.beta);
	_collisionPenaltySpin->setValue(cfg.collisionPenalty);
...
void RiskSensitiveIncomeLoadConfigWidget::setupUi()
{
	...
	auto* utilGroup = new QGroupBox(QString::fromLocal8Bit("Параметры рискочувствительности"), this);
	auto* utilLayout = new QFormLayout(utilGroup);
	_betaSpin = new QDoubleSpinBox();
	_collisionPenaltySpin = new QDoubleSpinBox();
	_gradientStepSpin = new QDoubleSpinBox();
	setupDouble(_betaSpin, 0.0, 100.0, 0.1, 4);
	setupDouble(_collisionPenaltySpin, 0.0, 1000.0, 0.1, 3);
	setupDouble(_gradientStepSpin, 0.0, 10.0, 0.01, 4);
	utilLayout->addRow(QString::fromLocal8Bit("Коэффициент риска (beta): "), _betaSpin);
	utilLayout->addRow(QString::fromLocal8Bit("Штраф за коллизию: "), _collisionPenaltySpin);
	...
	setConfig(RiskSensitiveLoadControllerConfig());
}
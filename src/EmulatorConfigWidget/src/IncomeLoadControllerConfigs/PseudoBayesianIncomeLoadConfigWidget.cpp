#include "PseudoBayesianIncomeLoadConfigWidget.h"
...
PseudoBayesianIncomeLoadConfigWidget::PseudoBayesianIncomeLoadConfigWidget(QWidget* parent)
	: QWidget(parent)
{
	setupUi();
}

PseudoBayesianLoadControllerConfig PseudoBayesianIncomeLoadConfigWidget::getConfig() const
{
	...
	cfg.maxStepUp = _maxStepUpSpin->value();
	cfg.maxStepDown = _maxStepDownSpin->value();
...
void PseudoBayesianIncomeLoadConfigWidget::setConfig(const PseudoBayesianLoadControllerConfig& cfg)
{
	_minProbabilitySpin->setValue(cfg.minProbability);
	_maxProbabilitySpin->setValue(cfg.maxProbability);
	_maxStepUpSpin->setValue(cfg.maxStepUp);
	_maxStepDownSpin->setValue(cfg.maxStepDown);
...
void PseudoBayesianIncomeLoadConfigWidget::setupUi()
{
	...
	_maxStepUpSpin = new QDoubleSpinBox();
	_maxStepDownSpin = new QDoubleSpinBox();
	...
	setupDouble(_maxStepUpSpin, 0.0, 1.0, 0.01, 4);
	setupDouble(_maxStepDownSpin, 0.0, 1.0, 0.01, 4);
	...
	probLayout->addRow(QString::fromLocal8Bit("Макс. шаг повышения pTx: "), _maxStepUpSpin);
	probLayout->addRow(QString::fromLocal8Bit("Макс. шаг понижения pTx: "), _maxStepDownSpin);
	...
	setConfig(PseudoBayesianLoadControllerConfig());
}
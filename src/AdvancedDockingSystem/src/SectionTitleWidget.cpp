...
void SectionTitleWidget::mouseReleaseEvent(QMouseEvent* ev)
{
	...
	if (_fw)
	{
		...
		if (sw)
		{
			cw->_dropOverlay->setAllowedAreas(AllAreas);
...
void SectionTitleWidget::mouseMoveEvent(QMouseEvent* ev)
{
	...
	if (_fw && (ev->buttons() & Qt::LeftButton))
	{
		...
		if (true)
		{
			...
			{
				cw->_dropOverlay->setAllowedAreas(TopDropArea);
			...
			{
				cw->_dropOverlay->setAllowedAreas(RightDropArea);
			...
			{
				cw->_dropOverlay->setAllowedAreas(BottomDropArea);
			...
			{
				cw->_dropOverlay->setAllowedAreas(LeftDropArea);

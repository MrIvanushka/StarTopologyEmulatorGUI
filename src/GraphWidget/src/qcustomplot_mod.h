...
private:
  ...
  friend class QCPAxisRect;
  friend class CustomQCPAxisRect;
};
...
public:
  ...
  QCPDataContainer();
  // ADDED BY UDPSENSER DEVELOPER
  QCPDataContainer(const QVector<DataType>& data, bool alreadySorted = false);
  ...
  // ADDED BY UDPSENDERMODEL DEVELOPER
  void removeByIndex(int index);
  // ADDED BY UDPSENDERMODEL DEVELOPER
  QVector<DataType> data() { return mData; }
  void clear();
  void sort();
...
template <class DataType>
QCPDataContainer<DataType>::QCPDataContainer(const QVector<DataType>& data, bool alreadySorted) :
    mAutoSqueeze(true),
    mPreallocSize(0),
    mPreallocIteration(0)
{
...
template <class DataType>
void QCPDataContainer<DataType>::removeByIndex(int index)
{
    if (index < 0 || index >= size()) {
        return;
    }
    QCPDataContainer::iterator it = begin() + index;
    if (it == begin())
        ++mPreallocSize; // don't actually delete, just add it to the preallocated block (if it gets too large, squeeze will take care of it)
    else
        mData.erase(it);
...
public:
  ...
  // ADDED BY UDPSENDERMODEL DEVELOPER
  friend QDataStream& operator << (QDataStream& stream, const QCPGraphData& data) {
      stream << data.key << data.value;
      return stream;
  }
  // ADDED BY UDPSENDERMODEL DEVELOPER
  friend QDataStream& operator >> (QDataStream& stream, QCPGraphData& data) {
      stream >> data.key >> data.value;
      return stream;
  }
